const eatSound = document.querySelector(".eating");
const gameBoard = document.querySelector(".game__board");
const ctx = gameBoard.getContext("2d");
const scoreText = document.querySelector(".game__score-text");
const highScoreText = document.querySelector(".game__high-score-text");
...
const SCREEN_THRESHOLD = 500; // limit of the screen width/hight
...
function gameStart() {
   if (
      ...
      window.innerHeight < SCREEN_THRESHOLD
   ) {
...
function changeDirectionByKey(keyCode) {
   ...
   const action = directionActions[keyCode];
   if (action) {
      action();
      pressingKeys();
   }
...
function snakeDirection(button) {
   const keyMap = {
      leftBtn: keyCodes.LEFT,
      upBtn: keyCodes.UP,
      rightBtn: keyCodes.RIGHT,
      downBtn: keyCodes.DOWN,
   };
